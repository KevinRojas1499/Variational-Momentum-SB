...
def dsm_loss(sde : SDEs.LinearSDE,data, cond=None):
    ...
    mean, var = sde.marginal_prob(data,shaped_t)
    noise = torch.randn_like(mean,device=data.device)
    perturbed_data = mean + var**.5 * noise
    return torch.mean((var**.5 * sde.backward_score(perturbed_data,times,cond) + noise)**2)
...
def standard_alternate_sb_loss(sde , data,optimize_forward, sampling_sde):
    n_times = 100
